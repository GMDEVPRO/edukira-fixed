...
const ERROR_TYPES = {
  401: { icon:'🔐', title:'Session expirée', desc:'Votre session a expiré. Reconnectez-vous.', action:'login' },
  403: { icon:'🚫', title:'Accès refusé',    desc:"Vous n'avez pas les permissions nécessaires.", action:'home' },
  404: { icon:'🔍', title:'Introuvable',     desc:'La ressource demandée est introuvable.', action:'retry' },
  500: { icon:'💥', title:'Erreur serveur',  desc:'Le serveur a rencontré un problème. Réessayez.', action:'retry' },
  0:   { icon:'📡', title:'Hors ligne',      desc:'Vérifiez votre connexion internet.', action:'retry' },
}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-12 px-4 text-center">
      ...
      <button onClick={handleAction}
        className="px-5 py-2.5 rounded-full text-white text-sm font-semibold transition-all hover:opacity-90 hover:-translate-y-px"
        style={{ background:'#1D9E75', boxShadow:'0 4px 14px rgba(29,158,117,0.35)' }}>
        {config.action === 'login' ? '🔐 Se reconnecter' : config.action === 'home' ? '← Accueil' : '🔄 Réessayer'}
      </button>
      {import.meta.env.DEV && (
